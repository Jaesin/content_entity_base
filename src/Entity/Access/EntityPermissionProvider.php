...
/**
 * @file
 *   Contains \Drupal\content_entity_base\Entity\Access\EntityPermissionProvider.
 */
...
class EntityPermissionProvider implements EntityPermissionProviderInterface, EntityHandlerInterface  {
  ...
  public function entityPermissions() {
    ...
    if (!empty($this->entity_type)) {
      ...
      $replacements = [
        ...
        '@entity_label' => $this->entity_type->getLabel(),
      ];
      ...
      if (($entity_type_storage = $this->entity_manager->getStorage($this->entity_type->getBundleEntityType()))
  ...
  protected function buildPermissions(EntityTypeBaseInterface $type) {
    ...
    $entity_definition = $this->entity_manager->getDefinition($entity_id);
    $type_id = $type->id();
    $type_params = [
